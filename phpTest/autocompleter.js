Vue.component("v-autocompleter", {
    ...
    methods:
    {
      findResultsDebounced : Cowboy.debounce(100, function findResultsDebounced() {
        fetch('http://localhost/KlaudiaToJa.github.io/phpTest/searchGoogle.php?name=' + this.value)
        ...
        <input 
            ...
            <div class="auto">         
                <div id="autocom" :class="[ value.length != 0 && filteredCities.length != 0 && kontrol == 1 ? 'autocompleter' : 'bez']">
                    <ul class="resultsBox">
                        <li class="pojedynczy" v-for="(city, index) in filteredCities" :key="city.name" @click="zmiana(city)" :class="{active : autocompleterIsActive && activeResult === index}">
                            <img class="lupaAuto" src="lupa.png">
                            <div class="pojWyn" v-html="city.name">
                            </div>  
                        </li>
                    </ul>
                </div>
